...
- [[Image Machine] Free browser-based image tools — convert (AVIF, WebP, JPEG XL, HEIC, PSD, RAW), compress to a target file size, resize, remove background, upscale, OCR, EXIF inspection and removal, GIF editing. 160+ tools in 24 languages, powered by WebAssembly (jSquash, ONNX Runtime Web, Tesseract.js); no server uploads.](https://imagemachinery.net/)
...
- [DBConvert Data Viewers](https://streams.dbconvert.com/parquet-viewer) - Free browser-based viewers for [Parquet](https://streams.dbconvert.com/parquet-viewer), [JSONL/NDJSON](https://streams.dbconvert.com/jsonl-viewer) and [SQLite](https://streams.dbconvert.com/sqlite-viewer) files. DuckDB-WASM runs the Parquet and JSONL queries and the official sqlite-wasm build reads the databases, all inside the tab: the file is opened through a browser file handle and never uploaded. Read-only (SELECT), no install, no account.
